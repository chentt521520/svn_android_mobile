...
import com.jc.lottery.fragment.VictorySettlementRecordFragment;
 ...
 * 结算列表页
 */
public class VictorySettlementListActivity extends BaseActivity implements ViewPager.OnPageChangeListener{
    ...
    // fragment的集合
    private ArrayList<VictorySettlementRecordFragment> list;
...
//        nameList = getIntent().getStringArrayListExtra("nameList");
        ...
        list = new ArrayList<VictorySettlementRecordFragment>();
        for (int i = 0; i < mTitle.length; i++) {
            if (i == 0) {
                list.add(new VictorySettlementRecordFragment("01"));
            }else {
                list.add(new VictorySettlementRecordFragment("02"));
            }
        }
...
//    }
    ...
    public class MySettlementViewPageAdapter extends FragmentPagerAdapter {
        ...
        private List<VictorySettlementRecordFragment> list;

        public MySettlementViewPageAdapter(FragmentManager fm, ArrayList<VictorySettlementRecordFragment> list) {
            super(fm);
            this.list = list;
        }
