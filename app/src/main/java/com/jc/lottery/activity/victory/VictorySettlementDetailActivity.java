...
import com.jc.lottery.bean.VictorySettlementDetailBean;
import com.jc.lottery.bean.req.pos_GetLotterySettlementDetails;
import com.jc.lottery.bean.req.pos_GetVictorySettlementDetails;
 ...
 * 胜负彩结算详情页
 */
public class VictorySettlementDetailActivity extends BaseActivity {
...
//    TextView tvPreviewCash;
    ...
    @BindView(R.id.tv_preview_total_settlement_settle_quota)
    ...
    private VictorySettlementDetailBean settlementGetDetailBean;
    ...
    public int getLayoutId() {
        return R.layout.activity_victory_settlement_detail;
    }
    ...
    private void getHttpInfo() {
        ...
        pos_GetVictorySettlementDetails.DataBean.SettlementInfo settlementInfo = new pos_GetVictorySettlementDetails.DataBean.SettlementInfo(orderCode);
        pos_GetVictorySettlementDetails pos_getSettlementRecord = new pos_GetVictorySettlementDetails(account_name, account_password, "3", settlementInfo);
        String s1 = new Gson().toJson(pos_getSettlementRecord);
        OkGo.<String>post(MyUrl.pos_GetSettleDetails)
                .upJson(s1)
...
//                            }
                            if (jsonObject.getString("code").equals("00000")) {
                                ...
                                settlementGetDetailBean = new Gson().fromJson(settlInfo.toString(), VictorySettlementDetailBean.class);
                                showData();
                            } else {
...
//        tvPreviewGame.setText(settlementGetDetailBean.getGameName());
        tvPreviewPayable.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getSale_money()) + getString(R.string.price_unit));
        tvPreviewExchangeCommission.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getPrize_commission()));
        tvPreviewCommissionRatio.setText(settlementGetDetailBean.getVictory_prize());
        tvPreviewSalesCommission.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getSales_commission()));
        tvPreviewSalesCommissionRatio.setText(settlementGetDetailBean.getVictory_sales());
...
//        tvPreviewPayable.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getTicketMoney()));
        tvPreviewTotalRewards.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getCash_money()) + getString(R.string.price_unit));
...
//        }
        if (settlementGetDetailBean.getSale_bets().equals("")) {
            tvPreviewSale.setText("0");
        } else {
            tvPreviewSale.setText(settlementGetDetailBean.getSale_bets());
        }

        if (settlementGetDetailBean.getSettle_status().equals("00")) {
...
//            tvPreviewTotalSettlementStatus.setText(getString(R.string.wait_shenhe));
        ...
        if (!settlementGetDetailBean.getAudit_time().equals("")) {
            tvPreviewTotalSettlementTime.setText(settlementGetDetailBean.getAudit_time());
        } else {
            tvPreviewTotalSettlementTime.setText("--");
        }
        if (settlementGetDetailBean.getTotal_money().equals("0")){
            tvPreviewTotalSettlementAmount.setTextColor(Color.rgb(255, 122, 0));
            tvPreviewTotalSettlementAmount.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getTotal_money()) + getString(R.string.price_unit));
        }else {
            if (settleStatus.equals("00")) {
                if (settlementGetDetailBean.getMoney_status().equals("00")) {
                    tvPreviewTotalSettlementAmount.setTextColor(Color.rgb(0,75,255));
                    tvPreviewTotalSettlementAmount.setText(getString(R.string.accounts_receivable) + " " + MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getTotal_money()) + getString(R.string.price_unit));
                } else {
                    tvPreviewTotalSettlementAmount.setTextColor(Color.rgb(253, 8, 8));
                    tvPreviewTotalSettlementAmount.setText(getString(R.string.payables) + " " + MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getTotal_money()) + getString(R.string.price_unit));
                }
            } else {
                if (settlementGetDetailBean.getMoney_status().equals("00")) {
                    tvPreviewTotalSettlementAmount.setTextColor(Color.rgb(253, 8, 8));
        ...
        int money = Integer.parseInt(settlementGetDetailBean.getTotalLimit()) - Integer.parseInt(settlementGetDetailBean.getSurplusAmount());
        if (money < 0){
            money = 0;
        }
        tvPreviewTotalSettlementSurplusQuota.setText(MoneyUtil.getIns().GetMoney((money) + "") + getString(R.string.price_unit));
        tvPreviewTotalSettlementUsedQuota.setText(MoneyUtil.getIns().GetMoney(settlementGetDetailBean.getSurplusAmount()) + getString(R.string.price_unit));
        ...
        if (settleStatus.equals("00") && settlementGetDetailBean.getSettle_status().equals("00")) {
