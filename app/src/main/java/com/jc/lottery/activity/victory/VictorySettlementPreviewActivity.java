...
import com.jc.lottery.bean.req.VictorySettleQueryBean;
import com.jc.lottery.bean.req.pos_LottoSettlement;
import com.jc.lottery.bean.req.pos_VictorySettlement;
import com.jc.lottery.bean.resp.SettleQueryInfo;
import com.jc.lottery.bean.resp.VictorySettleQueryInfo;
 ...
 * 胜负彩结算预览页面
 */
public class VictorySettlementPreviewActivity extends BaseActivity {
...
//    private String gameName = "";
    ...
    private VictorySettleQueryInfo settlementQueryBean = null;
    private boolean codeType = true;

    @Override
    public int getLayoutId() {
        return R.layout.activity_victory_settlement_preview;
    }
...
//        gameName = getIntent().getStringExtra("gameName");
        ...
        previewSettlementQueryHttp(endTime);
//        showData();
    }
...
//        tvPreviewGame.setText(gameName);
        ...
        tvPreviewCommissionRatio.setText(settlementQueryBean.getVictoryPrize());
        tvPreviewSalesCommission.setText(MoneyUtil.getIns().GetMoney(settlementQueryBean.getCommissionSales()));
        tvPreviewSalesCommissionRatio.setText(settlementQueryBean.getVictorySales());
...
//        tvPreviewCash.setText(settlementQueryBean.getCashBets());
        ...
        int money = Integer.parseInt(settlementQueryBean.getTotalLimit()) - Integer.parseInt(settlementQueryBean.getSurplusAmount());
        if (money < 0){
            money = 0;
        }
        tvPreviewTotalSettlementSurplusQuota.setText(MoneyUtil.getIns().GetMoney(money + "") + getString(R.string.price_unit));
        tvPreviewTotalSettlementUsedQuota.setText(MoneyUtil.getIns().GetMoney(settlementQueryBean.getSurplusAmount()) + getString(R.string.price_unit));
...
//        ClassicsHeader.REFRESH_HEADER_LASTTIME = getString(R.string.xlistview_header_last_time);
    ...
    //预览查询结算信息
    private void previewSettlementQueryHttp(Long time) {
        ProgressUtil.showProgressDialog(this, getString(R.string.waitting));
        String account_name = SPUtils.look(this, SPkey.username);
        String account_password = SPUtils.look(this, SPkey.password);
        VictorySettleQueryBean.SettlementInfo settlementInfo = new VictorySettleQueryBean.SettlementInfo(time);
        VictorySettleQueryBean.DataBean data = new VictorySettleQueryBean.DataBean(settlementInfo);
        VictorySettleQueryBean pos_getBookNumQuery = new VictorySettleQueryBean(account_name, account_password, "3", data);
        String s1 = new Gson().toJson(pos_getBookNumQuery);
        OkGo.<String>post(MyUrl.pos_GetSettlePreview)
                .upJson(s1)
                    ...
                    public void vOnSuccess(Response<String> response) {
                        ...
                        try {
                            ...
                            if (jsonObject.getString("code").equals("00000")) {
                                settlementQueryBean = new Gson().fromJson(response.body(), VictorySettleQueryInfo.class);
                                showData();
                            } else {
...
//        String deliveryCode = etSettlementOrder.getText().toString();
        String account_name = SPUtils.look(this, SPkey.username);
        String account_password = SPUtils.look(this, SPkey.password);
        pos_VictorySettlement.SettlementInfo settlementInfo = new pos_VictorySettlement.SettlementInfo(time + "", endTime + "", settlementQueryBean.getSaleMoney(), settlementQueryBean.getCashMoney(), settlementQueryBean.getCommission(), settlementQueryBean.getCommissionPrize(), settlementQueryBean.getCommissionSales(), settlementQueryBean.getVictoryPrize(), settlementQueryBean.getVictorySales(), settlementQueryBean.getCommissionSwitch(), settlementQueryBean.getTotalMoney(), settlementQueryBean.getMoneyStatus(), settlementQueryBean.getSaleBets(), settlementQueryBean.getTotalLimit(), settlementQueryBean.getSurplusAmount(),settlementQueryBean.getSettleQuota());
        pos_VictorySettlement.Data data = new pos_VictorySettlement.Data(settlementInfo);
        pos_VictorySettlement pos_getSettlement = new pos_VictorySettlement(account_name, account_password, "3", data);
        String s1 = new Gson().toJson(pos_getSettlement);
        OkGo.<String>post(MyUrl.pos_GetSettlement)
                .upJson(s1)
                .execute(new vStringCallback(this) {
                    ...
                    public void vOnSuccess(Response<String> response) {
                        ...
                        try {
                            ...
                            ToastUtils.showShort(jsonObject.getString("message"));
                            if (jsonObject.getString("code").equals("00000")) {
                                VictorySettlementActivity.instance.getBookInfoHttp();
                                finish();
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
