...
import com.jc.lottery.bean.req.pos_GetAuditOrder;
 ...
 * 领取详情页面
 */
public class ReceivingRecordsDetailActivity extends BaseActivity {
    ...
    TextView tvReceivingRealMoneys;
    ...
    @BindView(R.id.tv_receiving_real_state)
    TextView tvReceivingPaymentState;
    ...
    ScrollView svDetail;
    @BindView(R.id.bt_receiving_record_a_audit)
    Button btReceivingRecordAAudit;
    @BindView(R.id.lly_receiving_record_audit)
    PercentLinearLayout llyReceivingRecordAudit;
    @BindView(R.id.img_receiving_select_yes)
    ImageView imgReceivingSelectYes;
    @BindView(R.id.lly_receiving_select_yes)
    LinearLayout llyReceivingSelectYes;
    @BindView(R.id.img_receiving_select_no)
    ImageView imgReceivingSelectNo;
    @BindView(R.id.lly_receiving_select_no)
    LinearLayout llyReceivingSelectNo;
    @BindView(R.id.bt_receiving_pop_nos)
    Button btReceivingPopNos;
    @BindView(R.id.bt_receiving_pop_yess)
    Button btReceivingPopYess;
    @BindView(R.id.lly_receiving_pops)
    LinearLayout llyReceivingPops;
    ...
    public static ReceivingRecordsDetailActivity ins;
...
//        headerReceiving.setTimeFormat(new SimpleDateFormat(getString(R.string.xlistview_header_last_time) +" %s"));
        if (activityType.equals("00")) {
            ...
            tvRechargeDetailTitle.setText(R.string.delivery_records);
            tvReceivingState.setText(R.string.delivery_status);
            tvReceivingTime.setText(R.string.delivery_time);
        }
    }

    @OnClick({R.id.lly_reward_record_back, R.id.img_lottery_pop_one, R.id.lly_lottery_pop_zoom, R.id.bt_receiving_record_order, R.id.bt_receiving_record_cancel, R.id.bt_receiving_record_a_audit,R.id.bt_receiving_pop_yess,R.id.bt_receiving_pop_nos,R.id.lly_receiving_select_yes,R.id.lly_receiving_select_no})
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            case R.id.lly_reward_record_back:
                finish();
                break;
            case R.id.lly_receiving_select_yes:
                shType = 0;
                imgReceivingSelectYes.setImageResource(R.drawable.settle_yes);
                imgReceivingSelectNo.setImageResource(R.drawable.settle_no);
                break;
            case R.id.lly_receiving_select_no:
                shType = 1;
                imgReceivingSelectYes.setImageResource(R.drawable.settle_no);
                imgReceivingSelectNo.setImageResource(R.drawable.settle_yes);
                break;
            case R.id.bt_receiving_pop_yess:
                if (shType == 0){
                    getOrderAuditHttp(recordDetailsId, "00");
                }else {
                    getOrderAuditHttp(recordDetailsId, "01");
                }
                break;
            case R.id.bt_receiving_pop_nos:
                llyReceivingPops.setVisibility(View.GONE);
                break;
...
//                    startActivity(intent);
            ...
            case R.id.bt_receiving_record_cancel:
                getOrderCancelHttp(recordDetailsId);
                break;
            case R.id.bt_receiving_record_a_audit:
                llyReceivingPops.setVisibility(View.VISIBLE);
                break;
            default:
                break;
        }
...
//                                receivingOrderBean.setSendPerson(getRecordDetails.getString("sendPerson"));
                                if (getRecordDetails.has("shutState")){
                                    receivingOrderBean.setShutState(getRecordDetails.getString("shutState"));
                                }else {
                                    receivingOrderBean.setShutState("");
                                }
                                ...
                                if (receivingOrderBean.getStatus().equals("00")) {
                                    ...
                                    receivingActivationAdapter = new ReceivingActivationAdapter(ReceivingRecordsDetailActivity.this, receivingActivationBeanList);
...
//                                ReceivingRecordsActivity.ins.onRefresh();
    ...
    private void getOrderAuditHttp(String orderId, String type) {
        ProgressUtil.showProgressDialog(this, getString(R.string.waitting));
        String account_name = SPUtils.look(this, SPkey.username);
        String account_password = SPUtils.look(this, SPkey.password);
        pos_GetAuditOrder.OrderInfo orderInfo = new pos_GetAuditOrder.OrderInfo(orderId, type);
        pos_GetAuditOrder.DataBean dataBean = new pos_GetAuditOrder.DataBean(orderInfo);
        pos_GetAuditOrder pos_getLogisticsCancel = new pos_GetAuditOrder(account_name, account_password, "3", dataBean);
        String json = new Gson().toJson(pos_getLogisticsCancel);
        OkGo.<String>post(MyUrl.pos_GetCloseOrder)
                .upJson(json)
                .execute(new vStringCallback(this) {
...
//    }
    ...
    private void showInitView() {
        ...
        if (receivingOrderBean.getPayMoney().equals("0")) {
            ...
            tvReceivingRealMoneys.setText("--");
        } else {
            tvReceivingMoney.setText(MoneyUtil.getIns().GetMoney(receivingOrderBean.getPayMoney()) + getString(R.string.price_unit));
            tvReceivingRealMoneys.setText(MoneyUtil.getIns().GetMoney(receivingOrderBean.getPayMoney()) + getString(R.string.price_unit));
        }
...
//            tvReceivingRealMoneys.setText(MoneyUtil.getIns().GetMoney(receivingOrderBean.getPayMoneyReal()) + getString(R.string.price_unit));
        }
        if (activityType.equals("00")) {
            if (receivingOrderBean.getStatus().equals("00")) {
...
//                tvReceivingSix.setText(getString(R.string.to_be_dispatched));
        ...
        if (receivingOrderBean.getPayState().equals("00")) {
            ...
            tvReceivingPaymentState.setText(getString(R.string.successful_trade));
            ...
            tvReceivingPaymentState.setText(getString(R.string.failure_to_pay));
            ...
            tvReceivingPaymentState.setText(getString(R.string.to_be_paid));
            ...
            tvReceivingPaymentState.setText(getString(R.string.to_be_confirmed));
            tvReceivingThree.setTextColor(Color.rgb(255, 102, 0));
        }
...
//        role = "gly";
        if (receivingOrderBean.getShutState().equals("00")) {
            tvReceivingSix.setText(getString(R.string.in_closing));
            tvReceivingSix.setTextColor(Color.rgb(255, 102, 0));
            if (role.equals("dls")) {
                llyReceivingRecordAudit.setVisibility(View.VISIBLE);
            }
        } else {
            llyReceivingRecordAudit.setVisibility(View.GONE);
            if (!receivingOrderBean.getStatus().equals("00") && !receivingOrderBean.getStatus().equals("05")) {
