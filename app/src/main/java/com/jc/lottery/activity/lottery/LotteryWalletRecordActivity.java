 ...
 * 钱包流水页
 */
public class LotteryWalletRecordActivity extends BaseActivity {
    ...
    @BindView(R.id.tv_receiving_select_wallet)
    TextView tvReceivingSelectWallet;
    @BindView(R.id.tv_receiving_select_fixed)
    TextView tvReceivingSelectFixed;
    @BindView(R.id.lly_receiving_select_container_one)
    LinearLayout llyReceivingSelectContainerOne;
    @BindView(R.id.lly_lottery_top)
    LinearLayout llyLotteryTop;
    private WalletRecordAdapter paymentRecordAdapter;
    private List<WalletListBean> payListBeanList = new ArrayList<WalletListBean>();
    ...
    private String end = "";
    private String statusSelect = "01";

    @Override
    public int getLayoutId() {
        return R.layout.activity_lottery_wallet_record;
    }
...
//        role = "gly";
        if (role.equals("gly")) {
            llyLotteryTop.setVisibility(View.GONE);
        }else if (role.equals("fxs")) {
            statusSelect = "02";
            llyLotteryTop.setVisibility(View.GONE);
        }else {
            llyLotteryTop.setVisibility(View.VISIBLE);
        }
    }
...
//        tvReceivingSelectEnd.setText("");
    ...
    public void initData() {
        paymentRecordAdapter = new WalletRecordAdapter(this);
        getHttpInfo();
    }
...
//            return String.valueOf(sdf.parse(date).getTime());
    ...
    private void getHttpInfo() {
        ...
        final pos_GetWalleQueryInfo.WalleInfo walleInfo = new pos_GetWalleQueryInfo.WalleInfo(statusSelect, "", pageNo + "", starts, ends, TimeUtils.get10IntTimeStamp() + "");
        pos_GetWalleQueryInfo.DataBean dataBean = new pos_GetWalleQueryInfo.DataBean(walleInfo);
        pos_GetWalleQueryInfo pos_getWalleQueryInfo = new pos_GetWalleQueryInfo(account_name, dataBean);
        String s1 = new Gson().toJson(pos_getWalleQueryInfo);
        OkGo.<String>post(MyUrl.pos_GetWalleQuery)
                .upJson(s1)
                    ...
                    public void vOnSuccess(Response<String> response) {
                        ...
                        try {
                            ...
                            if (jsonObject.getString("code").equals("00000")) {
                                ...
                                Type listType = new TypeToken<List<WalletListBean>>() {
                                }.getType();
                                List<WalletListBean> list = new Gson().fromJson(jsonObject.getJSONArray("walletList").toString(), listType);
                                for (int i = 0; i < list.size(); i++) {
                                    list.get(i).setIndex(num);
                                    num++;
                                }
                                payListBeanList.addAll(list);
                                paymentRecordAdapter.setList(payListBeanList);
...
//                        ToastUtils.showShort("response:" + response.body());
    ...
    @OnClick({R.id.lly_manual_scanner_back, R.id.lly_reward_record_select, R.id.lly_receiving_select, R.id.tv_receiving_select_start, R.id.tv_receiving_select_end, R.id.btn_receiving_select_submit, R.id.btn_receiving_select_reset, R.id.lly_receiving_click,R.id.tv_receiving_select_wallet,R.id.tv_receiving_select_fixed})
...
//                getRecordHttp();
                ...
                new ViewAnimationUtil().setHideAnimation(llyReceivingSelect, 300, mAnimation);
                break;
            case R.id.tv_receiving_select_wallet:
                statusSelect = "01";
                tvReceivingSelectWallet.setBackgroundResource(R.drawable.reward_et_red_bg);
                tvReceivingSelectFixed.setBackgroundResource(R.drawable.reward_et_bg);
                tvReceivingSelectWallet.setTextColor(Color.WHITE);
                tvReceivingSelectFixed.setTextColor(Color.GRAY);
                break;
            case R.id.tv_receiving_select_fixed:
                statusSelect = "02";
                tvReceivingSelectWallet.setBackgroundResource(R.drawable.reward_et_bg);
                tvReceivingSelectFixed.setBackgroundResource(R.drawable.reward_et_red_bg);
                tvReceivingSelectWallet.setTextColor(Color.GRAY);
                tvReceivingSelectFixed.setTextColor(Color.WHITE);
                break;
        }
    }
