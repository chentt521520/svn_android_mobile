        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    LogUtils.e("   +++++++++++++++   s收到返回的状态   " + cnt + "    " + result);
                    String status = LotteryApplication.getContext().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " " + LotteryApplication.getContext().getString(R.string.str_printer_out_of_paper);
                                    Log.e("zwc", "111111111111111111111111111: ");
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " " + LotteryApplication.getContext().getString(R.string.str_printer_open_cover);
                                    Log.e("zwc", "222222222222222222222222222: ");
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " " + LotteryApplication.getContext().getString(R.string.str_printer_error);
                                    Log.e("zwc", "33333333333333333333333333333333: ");
                                }
                                System.out.println(LotteryApplication.getContext().getString(R.string.str_state) + status);
//                                Utils.toast(LotteryApplication.getContext(), status);
                                // TODO: 2018/9/29  打印机连接正常
                            }
                        }
